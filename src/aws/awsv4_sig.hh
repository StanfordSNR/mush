//
// Created by ziheng on 6/28/21.
//

#ifndef MUSH_AWSV4_SIG_HH
#define MUSH_AWSV4_SIG_HH

#endif //MUSH_AWSV4_SIG_HH
...
namespace AWSv4Sig{
    ...
    string
    sha256buf_to_string_(const unsigned char *buf) {
    ...
    vector<uint8_t>
    derive_signing_key_(const string &secret,
                        const string &date,
                        const string &region,
    ...
    void
    sign_request(const std::string & first_line,
        ...
        std::string request_date =  AWSv4Sig::x_amz_date_( time( 0 ) ) ;
        headers.headers["x-amz-date"] = request_date;
